...
function chapter1(): Paragraph[] {
  return [
    ...
    emptyLine(),
    bodyTextMulti([tr("・出勤、退勤、休憩の打刻（タイムカードの代わり）")], { indent: { left: convertMillimetersToTwip(5) } }),
...
function chapter2(): Paragraph[] {
  return [
    ...
    bodyTextMulti([
      tr("　　入力するアドレス: ", { size: 22 }),
      tr("http://[サーバーIP]:3000", { bold: true, size: 22 }),
    ]),
    tip("アドレスは、会社から届いたメールや掲示に記載されています。わからない場合は、情報システム部にお問い合わせください。"),
...
function chapter4(): Paragraph[] {
  return [
    ...
    emptyLine(),
    heading2("4-4　昼休みの休憩打刻"),
    bodyText("昼休みなどの休憩に入るときと、休憩が終わったときに打刻します。"),
    emptyLine(),
    heading3("休憩に入るとき"),
    step("①", "KINTAIアプリの打刻画面を開きます。"),
    step("②", "黄色の【☕ 休憩開始】ボタンをタップします。"),
    step("③", "ステータスが「休憩中」に変わったら完了です。"),
    emptyLine(),
    heading3("休憩が終わったとき"),
    step("①", "KINTAIアプリの打刻画面を開きます。"),
    step("②", "黄色の【▶ 休憩終了】ボタンをタップします。"),
    step("③", "ステータスが「勤務中」（または「在宅勤務中」）に戻ったら完了です。"),
    emptyLine(),
    tip("休憩時間は自動で計算されますので、正確に打刻してください。"),
    emptyLine(),
    heading2("4-5　退勤打刻"),
  ...
  return [
    ...
    makeTable(
      ...
      [
        ...
        ["🏠 在宅勤務", "紫", "自宅で仕事を始めるとき"],
        ["🔴 退勤", "赤", "仕事が終わったとき"],
        ["☕ 休憩開始", "黄", "昼休みなどの休憩に入るとき"],
        ["▶ 休憩終了", "黄", "休憩が終わって仕事に戻るとき"],
      ]
    ),
    emptyLine(),
    ...
    emptyLine(),
    heading2("7-3　ボトムメニュー（画面下部のメニュー）"),
    bodyText("画面の一番下に表示されているメニューバーを「ボトムメニュー」といいます。各アイコンをタップすると、それぞれの画面に移動できます。"),
    emptyLine(),
    makeTable(
      ["アイコン", "名前", "機能"],
      [
    ...
    emptyLine(),
    tip("画面を切り替えるには、ボトムメニューのアイコンをタップするだけです。いつでも好きな画面に移動できます。"),
