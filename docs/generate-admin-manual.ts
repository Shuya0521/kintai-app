...
function chapter1(): Paragraph[] {
  return [
    ...
    bodyText("従業員ポータルとは別のURLで動作します。データベースにはTurso Cloudを使用しています。"),
    emptyLine(),
    bodyText("管理者ポータルでは、以下の機能を利用できます。"),
    bullet("休暇申請の承認・却下"),
    bullet("全社員の月次勤怠データの閲覧・Excel出力・CSV出力"),
    ...
    bodyTextMulti([
      tr("　URL: ", { bold: true }),
      tr("https://kintai-admin.onrender.com", { bold: true }),
    ]),
    emptyLine(),
    tip("従業員ポータルとは別のURLです。ブックマークしておくことをおすすめします。"),
    emptyLine(),
    ...
    emptyLine(),
    makeTable(
      ["ロール", "管理画面アクセス", "承認権限", "Excel/CSV出力", "社員管理", "システム設定"],
      [
        ["システム管理者", "○", "○", "○", "○", "○"],
        ["取締役", "○", "○", "○", "○", "○"],
        ["統括部長", "○", "○", "○", "○", "×"],
...
function chapter2(): Paragraph[] {
  return [
    ...
    heading2("2-1　ログイン手順"),
    step("①", "ブラウザで管理者ポータルのURL（https://kintai-admin.onrender.com）にアクセスします。"),
    step("②", "ログイン画面が表示されます。"),
    step("③", "「メールアドレス」欄に、管理者権限を持つアカウントのメールアドレスを入力します。"),
    step("④", "「パスワード」欄にパスワードを入力します。"),
    step("⑤", "必要に応じて「メールアドレスを記憶する」チェックボックスにチェックを入れます。チェックを入れると、次回以降メールアドレスが自動入力されます。"),
    ...
    heading2("2-2　ログアウト"),
    bodyText("デスクトップ版: サイドバー下部の【ログアウト】ボタンをクリックします。"),
    bodyText("モバイル版: 画面下部のナビゲーションバーの「その他」メニューを開き、【ログアウト】をタップします。"),
    emptyLine(),
...
function chapter5(): Paragraph[] {
  return [
    ...
    heading2("5-6　CSV出力"),
    bodyText("表示中の勤怠データをCSVファイル（.csv）としてダウンロードすることもできます。"),
    bodyText("【CSV出力】ボタンは【Excel出力】ボタンの横に配置されています。"),
    emptyLine(),
    step("①", "表示したい月・部署を設定します。"),
    step("②", "【CSV出力】ボタンをクリックします。"),
    step("③", "CSVファイルが自動的にダウンロードされます。"),
    emptyLine(),
    bodyText("CSV出力の特徴:"),
    bullet("月・部署フィルタの設定がそのまま適用されます"),
    bullet("BOM付きUTF-8形式で出力されるため、Excelで開いても文字化けしません"),
    bullet("他のシステムへのデータ連携やインポート用途に適しています"),
    emptyLine(),
    caution("CSV出力はExcel出力と同様、システム管理者・取締役・統括部長・部長のみ利用可能です。"),
    emptyLine(),
  ...
  return [
    ...
    makeTable(
      ...
      [
        ...
        ["河村（取締役）", "○", "○（全機能）", "管理画面+承認+Excel/CSV出力"],
        ["鈴木（部長）", "○", "○（承認・勤怠・Excel/CSV）", "設定以外の管理機能"],
        ["河田（部長）", "○", "○（承認・勤怠・Excel/CSV）", "設定以外の管理機能"],
        ["串間（課長）", "○", "×", "従業員ポータルのみ"],
        ["笠間（リーダー）", "○", "×", "従業員ポータルのみ"],
      ]
    ),
